...
void myprint(int *ptr, int max, float *normalX, float *normalY, float *normalZ, float maxAngle, int *faces, float *point1X, float *point1Y, float *point2X, float *point2Y, float *point3X, float *point3Y, float *point1Z, float *point2Z, float *point3Z);
int intpoint_inside_trigon(float sx, float sy, float ax, float ay, float bx, float by, float cx, float cy);
    ...
    for (i = 0; i < max; i++) 
    {
      ...
      if((angle*180/M_PI) < maxAngle || ((dot > 0.9999) && (dot < 1.0001)))
      {
        ...
        for (j = 0; j < max; j++)       
        {
          ...
          {
            if(intpoint_inside_trigon(triangleCenterX,triangleCenterY,point1X[j],point1Y[j],point2X[j],point2Y[j],point3X[j],point3Y[j]) == 1)
            {
              //printf("OK");
              faces[i] = 0;
              flagBreak = 1;
